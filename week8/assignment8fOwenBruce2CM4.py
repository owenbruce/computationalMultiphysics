...
FlexCode = """TITLE 'EfieldConductor & Dielectric'     { the problem identification }
...
PLOTS            { save result displays }
 grid(x+mag*u, y+mag*v)
  ...
  elevation(Globalmax(Temp)) from (0,0) to (L,0) PrintOnly Export Format '#t#b#1' file = '%s_V.txt'
SUMMARY
summary
report Vmax
...
FlexFileName = "D:\\EngPhys\\2CM4\\Week8\\assignment8fOwenBruce2CM4.pde"    #Change to whatever folder and file you want
...
Vmax = 0.1
Vstep = 0.01
VRange = sp.arange(Vmin,Vmax,Vstep)
...
while Vstep>=0.01:
    for V in VRange:
        with open(FlexFileName, "w") as f:
            print(FlexCode%(round(V,7),round(V,7)), file=f)
    ...
    for V in VRange:
        try:
            with open("D:\\EngPhys\\2CM4\\Week8\\assignment8fOwenBruce2CM4_output\\"+str(round(V,7))+"_V.txt") as f:   #Change to whatever folder the output is in
                data=sp.loadtxt(f, skiprows=8)
            Temp=data[:,1]
            if abs(Temp[-1]-100) < accTemp:
                accTemp = abs(Temp[-1]-100)
                maxV = V
            print('With a Vmax of {V}V, we get a max temp of {maxTemp}'.format(V=round(V,7), maxTemp=Temp[-1]))
        except:
            print("missing: ", round(V,7))
...
print("")
print("The most accurate temperature is", accTemp + 100, " resulting from a Voltage of:", round(maxV,7))
print("")
